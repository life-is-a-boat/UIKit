...
static NSString * const JMC_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const JMCRouterParameterURL = @"JMCRouterParameterURL";
NSString *const JMCRouterParameterCompletion = @"JMCRouterParameterCompletion";
NSString *const JMCRouterParameterUserInfo = @"JMCRouterParameterUserInfo";
...
@implementation JMCRouter

+ (instancetype)sharedInstance {
    static JMCRouter *instance = nil;
...
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(JMCRouterHandler)handler
{
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        JMCRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[JMCRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[JMCRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    JMCRouter *router = [JMCRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL];
    JMCRouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(JMCRouterObjectHandler)handler
{
    [[self sharedInstance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(JMCRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(JMCRouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url
{
    ...
    parameters[JMCRouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:JMC_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if (!URL.length) {
            [pathComponents addObject:JMC_ROUTER_WILDCARD_CHARACTER];
        }
    }
